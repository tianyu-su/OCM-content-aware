...
class DataLoaderPolyvore(Dataloader):
    ...
    def __init__(self, datadir="data/polyvore/utils_benchmark/dest", polyvore_split="nondisjoint"):
        super(DataLoaderPolyvore, self).__init__(path=osp.join(datadir, polyvore_split + "/"))
    ...
    def init_phase(self, phase):
        ...
        retrieval_questions_file = path_dataset + 'retrieval_questions_test.json'
        # questions_file_resampled = questions_file.replace('questions', 'questions_RESAMPLED')
        with open(questions_file) as f:
            self.questions = json.load(f)

        with open(retrieval_questions_file) as fp:
            self.retrieval_questions = json.load(fp)
    ...
    def get_phase(self, phase):
        ...
        n_train_neg = eval_pos_labels.shape[
            0]  # set the number of negative training edges that will be needed to sample at each iter
    ...
    def get_test_retrieval_questions(self, resampled=False):
        """
        ...
        valid = []
        q_id = 0
        questions = self.retrieval_questions if not resampled else self.questions_resampled
        ...
        assert len(self.retrieval_questions) == max(q_ids) + 1
    ...
    def yield_test_retrieval_questions_K_edges(self, resampled=False, K=1, subset=False, expand_outfit=False):
        """
        Yields questions, each of them with their own adj matrix.
        Each node on the question will be expanded to create K edges, so the adj
        matrix will have K*N edges.
