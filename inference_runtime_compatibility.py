...
def test_compatibility(args):
    ...
    def eval():
        ...
        val_avg_loss, val_acc, conf, pred = sess.run([model.loss, model.accuracy, model.confmat, model.predict()],
    ...
    with tf.Session() as sess:
        ...
        for outfit in dl.comp_outfits:
            ...
            one_el_time = time.time() - before_item
            author_runtim_avg.append(one_el_time)
            print("Total Elapsed: {:.4f}".format(one_el_time))
            count += 1

            preds.append(predicted_score)
            labels.append(score)

        mine_total_rumtime = time.time() - mine_start_time

        def info(ffp, arg):
            print(arg)
            ffp.write(arg)

        with open("test_auc_time_{}.txt".format(int(time.time()*10**9) % 19987), 'w') as fp:
            # fp.write("total: {} \n".format(mine_total_rumtime))
            # fp.write("data size: {} \n".format(len(dl.comp_outfits)))
            # fp.write("mine avg: {} \n".format(mine_total_rumtime / len(dl.comp_outfits)))
            # fp.write("author avg: {} \n".format(np.mean(author_runtim_avg)))
            info(fp, "total: {} \n".format(mine_total_rumtime))
            info(fp, "data size: {} \n".format(len(dl.comp_outfits)))
            info(fp, "mine avg: {} \n".format(mine_total_rumtime / len(dl.comp_outfits)))
            info(fp, "author avg: {} \n".format(np.mean(author_runtim_avg)))
